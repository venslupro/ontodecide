...
export class D1DecisionRepository implements IDecisionRepository {
  ...
  public async findByHash(tenantId: string, promptHash: string): Promise<DecisionRecord | null> {
    const row = await this.db
      .select()
      .from(decisions)
      .where(and(eq(decisions.tenant_id, tenantId), eq(decisions.prompt_hash, promptHash)))
      .orderBy(desc(decisions.created_at))
      .limit(1)
      .get();
