...
export async function listUsersHandler(
  ...
  return c.json(ok({total, page, size, list: items},
...
export async function updateStatusHandler(
  ...
  if (body?.is_active === undefined) {
    return c.json(
        fail(ERROR_CODES.VALIDATION_FAILED, 'is_active is required.'),
        400,
    );
  }
...
export async function resetPasswordHandler(
  ...
  const temporaryPassword =
    await service.resetPassword(id, auditContext(c));
  return c.json(ok({temporary_password: temporaryPassword},
      c.req.header(HEADERS.TRACE_ID)), 200);
}
...
export async function listAuditHandler(
  ...
  if (!tenantId) {
    return c.json(
        fail(ERROR_CODES.AUTH_FORBIDDEN, 'Missing tenant id.'),
        403,
    );
  }
  const page = parseInt(c.req.query('page') ?? '1', 10);
...
/** GET /admin/config */
export async function listConfigHandler(
    c: Context,
    service: UserManagementService,
) {
  return c.json(ok(await service.getAllConfig(),
      c.req.header(HEADERS.TRACE_ID)), 200);
}

/** PUT /admin/config  body: { key: string, value: string } */
export async function setConfigHandler(
    c: Context,
    service: UserManagementService,
) {
  const body = await c.req.json();
  if (!body?.key || body.value === undefined) {
    return c.json(
        fail(ERROR_CODES.VALIDATION_FAILED, 'key and value are required.'),
        400,
    );
  }
