...
export async function triggerCleanupHandler(c: Context) {
  ...
  if (role !== 'admin') {
    return c.json(fail(ERROR_CODES.AUTH_FORBIDDEN, 'Admin role required.'), 403);
  }
...
export async function cleanupStatusHandler(c: Context) {
  ...
  if (!taskId) {
    return c.json(fail(ERROR_CODES.VALIDATION_FAILED, 'Task id is required.'), 400);
  }
  const record = await readTask(env, taskId);
  if (!record) {
    return c.json(fail(ERROR_CODES.CLEANUP_TASK_NOT_FOUND, `Task ${taskId} not found.`), 404);
  }
  return c.json(
    ok(
      {
        ...
        processed: record.progress.filter((p) => p.state === 'succeeded' || p.state === 'failed')
          .length,
