...
export const apiErrorSchema = z.object({
  code: z
      .string()
      .openapi({description: 'Stable error code, e.g. AUTH_INVALID_CREDENTIALS.'}),
  message: z
      .string()
      .openapi({description: 'Human-readable message in the user locale.'}),
  details: z
...
export const apiResponseSchema = <T extends z.ZodTypeAny>(data: T) =>
  z.object({
    success: z
        .boolean()
        .openapi({description: 'Whether the request succeeded.'}),
    data: data
        .optional()
        .openapi({description: 'Response payload on success.'}),
    error: apiErrorSchema
        .optional()
        .openapi({description: 'Error details on failure.'}),
    traceId: z
        .string()
        .optional()
...
export const paginatedResponseSchema = <T extends z.ZodTypeAny>(item: T) =>
  z.object({
    ...
    page: z
        ...
        .int()
        .positive()
        .openapi({description: 'Current page number (1-based).'}),
    size: z
        .number()
        .int()
        .positive()
        .openapi({description: 'Page size used for the query.'}),
    list: z
        .array(item)
        .openapi({description: 'Records on the current page.'}),
  });
...
export const pageQuerySchema = z.object({
  page: z
      ...
      .positive()
      .optional()
      .openapi({description: 'Page number (1-based).'}),
  size: z
      .number()
      .int()
      .positive()
...
export const userPublicSchema = z.object({
  ...
  tenant_id: z
      .string()
      .openapi({description: 'Owning tenant id, format tenant_xxxx.'}),
  username: z.string().openapi({description: 'Login username.'}),
  email: z
      .string()
      .nullable()
      .openapi({description: 'Contact email, or null when unset.'}),
  role: userRoleSchema.openapi({description: 'Authorization role.'}),
  is_active: z
      .boolean()
      .openapi({description: 'Whether the account may authenticate.'}),
  is_data_cleared: z
      .boolean()
...
export const credentialResultSchema = z.object({
  ...
  tenant_id: z
      .string()
      .openapi({description: 'Tenant the user belongs to.'}),
  username: z.string().openapi({description: 'Login username.'}),
...
export const ontologyTypeSchema = z.object({
  id: z
      .string()
      .openapi({description: 'Stable id, e.g. asset.'}),
  name: z.string().openapi({description: 'Human-readable name.'}),
  properties: z
      .array(z.string())
      .openapi({description: 'Property keys allowed on this type.'}),
  relations: z
      .array(z.string())
      .openapi({description: 'Relation types allowed on this type.'}),
  created_at: z
      .string()
      .optional()
      .openapi({description: 'ISO-8601 creation timestamp.'}),
...
export const entityNodeSchema = z.object({
  ...
  tenant_id: z
      .string()
      .openapi({description: 'Owning tenant id.'}),
  type: z
      .string()
      .openapi({description: 'Ontology type label of the entity.'}),
  attributes: z
      .record(z.string(), z.unknown())
      .openapi({description: 'Arbitrary key/value attributes of the entity.'}),
  source: z
      .string()
      .openapi({description: 'Source identifier that produced the entity.'}),
  confidence: z
      .number()
      .openapi({description: 'Confidence score of the entity (0..1).'}),
  timestamp: z
      .string()
      .openapi({description: 'ISO-8601 timestamp of the entity.'}),
});
...
export const entityRelationSchema = z.object({
  type: z
      .string()
      .openapi({description: 'Edge type label, e.g. LOCATED_AT.'}),
  ...
  relations: z
      .array(
          z.object({
            type: z
                .string()
                .openapi({description: 'Relation type label.'}),
...
export const exploreRequestSchema = z.object({
  entityId: z
      .string()
      .openapi({description: 'Root entity id to start exploration from.'}),
  depth: z
      .number()
      .int()
      .min(1)
...
export const ingestPayloadSchema = z.object({
  ...
  entities: z
      .array(entityNodeSchema)
      .openapi({description: 'Entities to upsert.'}),
  relations: z
      .array(entityRelationSchema)
      .openapi({description: 'Relations to upsert.'}),
  source: z
      .string()
      .openapi({description: 'Source identifier (webhook id, file name, etc.).'}),
});
...
export const scenarioToneSchema = z.enum([
  'optimistic',
  'pessimistic',
  'neutral',
]);
...
export const scenarioResultSchema = z.object({
  ...
  topic: z
      .string()
      .openapi({description: 'Topic or question the scenario was generated for.'}),
  scenarios: z
      .array(
          z.object({
  ...
  generatedAt: z
      .string()
      .openapi({description: 'ISO-8601 generation timestamp.'}),
...
export const recommendationSchema = z.object({
  ...
  topic: z
      .string()
      .openapi({description: 'Topic the recommendation addresses.'}),
  priority: z
      .enum(['low', 'medium', 'high', 'critical'])
      .openapi({description: 'Priority of the recommendation.'}),
  confidence: z
      .number()
      .openapi({description: 'Confidence score of the recommendation (0..1).'}),
  rationale: z
      .string()
      .openapi({description: 'Reasoning behind the recommendation.'}),
  steps: z
      .array(
          z.object({
            order: z
                ...
                .int()
                .positive()
                .openapi({description: '1-based execution order.'}),
            action: z.string().openapi({description: 'Action to perform.'}),
            expectedOutcome: z
                .string()
                .openapi({description: 'Expected outcome of the action.'}),
          }),
      )
...
export const agentTaskSchema = z.object({
  ...
  result: z
      .string()
      .optional()
      .openapi({description: 'Result text once the task finished.'}),
  startedAt: z
      .string()
      .optional()
...
export const agentStateSchema = z.object({
  ...
  tasks: z
      ...
      .openapi({description: 'Tasks planned for the goal.'}),
  status: z
...
export const loginSchema = z.object({
  ...
  password: z
      .string()
      .openapi({description: 'Plaintext password (transmitted over TLS).'}),
});
...
export const refreshSchema = z.object({
  refreshToken: z
      .string()
      .openapi({description: 'Refresh token issued at login.'}),
});
...
export const authTokensSchema = z.object({
  ...
  refreshToken: z
      .string()
      .openapi({description: 'Long-lived refresh token.'}),
  expiresIn: z
      .number()
      .int()
      .positive()
...
export const ingestFileSchema = z.object({
  objectKey: z
      .string()
      .openapi({description: 'Tenant-scoped object key under R2.'}),
  format: z
...
export const ingestJobEnqueuedSchema = z.object({
  ...
  status: z
      .literal('queued')
      .openapi({description: 'Initial job status.'}),
});
...
export const scenarioRequestSchema = z.object({
  topic: z
      .string()
      .openapi({description: 'Free-form topic or question to simulate.'}),
  context: z
      .string()
      .optional()
  ...
  provider: llmProviderSchema
      .optional()
      .openapi({description: 'Optional provider override.'}),
});
...
export const cleanupStatusSchema = z.object({
  ...
  processed: z
      ...
      .int()
      .nonnegative()
      .openapi({description: 'Tenants processed so far.'}),
  total: z
      .number()
      .int()
      .nonnegative()
      .openapi({description: 'Total tenants to process.'}),
  startedAt: z
      .string()
      .optional()
      .openapi({description: 'ISO-8601 start timestamp.'}),
